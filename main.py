...
def start(message):
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="المالك", url="https://t.me/@i7cy7")
			bu2 = types.InlineKeyboardButton(text="Use The Bot Free -استخدام البوت مجانا ", url=f"https://t.me/cc_chk")
			keyboard.add(contact_button)
			photo_url = 'https://t.me/@i7cy7/30231'
	...
	''',reply_markup=keyboard)
		...
		photo_url = f'https://t.me/i7cy7/30231'
...
def process_payment(message):
    ...
    bot.send_invoice(
        ...
        title="خدمة البوت",
        description=f"ادفع {SERVICE_COST} 50 للاستفادة من كود 24 ساعه في البوت",
        provider_token="",
        currency="XTR",
        prices=prices,
			...
			with open(cooo, 'r') as file:
				...
				for cc in lino:
					...
					bot.edit_message_text(chat_id=call.message.chat.id, 
					  ...
					  text=f''' 𝙂𝙖𝙩𝙚𝙬𝙖𝙮 {gate}
 𝗕𝘆 ⇾ <a href='tg://user?id=5168499996'>DRGAM</a>''', reply_markup=mes)
		...
		try:
			with open(abdo, 'r') as file:
				...
				for cc in lino:
					...
					cm1 = types.InlineKeyboardButton(f"• {cc} •", callback_data='u8')
					status = types.InlineKeyboardButton(f"• 𝙎𝙏𝘼𝙏𝙐𝙎 ➜ {last} •", callback_data='u8')
					cm3 = types.InlineKeyboardButton(f"• 𝙋𝘼𝙎𝙎ED✅ ➜ [ {ch} ] •", callback_data='x')
...
𝘽𝙤𝙩 𝘽𝙮 𝗕𝗼𝘁 𝗕𝘆 ⇾ <a href='tg://user?id=5168499996'>DRGAM</a>''', reply_markup=mes)
...
Bot by : 𝗕𝗼𝘁 𝗕𝘆 ⇾ <a href='tg://user?id=5168499996'>DRGAM</a>
Taken {"{:.1f}".format(execution_time)} secounds .</b>'''
					msgc=f'''<b> ϟ - OTP Card ✅
#OTP
<a href='tg://user?id=5168499996'>[⌥]</a>Card ->  <code>{cc}</code>
<a href='tg://user?id=5168499996'>[⌥]</a> Status -> {last}
<a href='tg://user?id=5168499996'>[⌥]</a> Gate -> {gate}

card Info -> <code>{card_type} </code>- <code>{brand}</code>
Bank -> <code>{bank}</code>
Counrty -> <code>{country}</code> - {country_flag} 

<a href='tg://user?id=5168499996'>[⌥]</a> Time -> {"{:.1f}".format(execution_time)} Seconds. [VIP]
<a href='tg://user?id=5168499996'>[⌥]</a> Programmer -> @i7cy7⚡</b>'''
...
Bank -> <code>{bank}</code>
Counrty -> <code>{country}</code> - {country_flag} 
Bot by : 𝗕𝗼𝘁 𝗕𝘆 ⇾ <a href='tg://user?id=5168499996'>𝘿𝙍𝙂𝘼𝙢</a>
Taken {"{:.1f}".format(execution_time)} secounds .</b>'''
...
Bank -> <code>{bank}</code>
Counrty -> <code>{country}</code> - {country_flag} 
Dev : 𝗕𝗼𝘁 𝗕𝘆 ⇾ <a href='tg://user?id=5168499996'>DRGAM</a>
Taken {"{:.1f}".format(execution_time)} secounds .</b>'''
	msgf=f'''<b> Approved ✅
[ϟ] Card ->  <code>{cc}</code>
[ϟ] Status -> {last}
...
def respond_to_vbv(message):
  ...
  "timer": "none",
	...
	if BL == '𝗙𝗥𝗘𝗘':
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/i7cy7")
		ahmed = types.InlineKeyboardButton(text="✨ 𝘾𝙃𝘼𝙉𝙉𝙀𝙇  ✨", url="https://t.me/i7cy7")
